...
preflight_checks() {
    ...
    if [[ ! -x "$DIALOG_BIN" ]]; then
        ...
        if [[ ! -x "$DIALOG_BIN" ]]; then
            ...
            osascript -e 'display alert "Installation Failed" message "swiftDialog could not be installed automatically. Please email itsupport@example.com for assistance." as critical'
            exit 1
        fi
    ...
    if ! sudo -u "$CONSOLE_USER" security find-generic-password \
        ...
        if ! sudo -u "$CONSOLE_USER" security find-generic-password \
            ...
            osascript -e 'display alert "Setup Failed" message "This Mac could not be configured for log upload automatically. Please email itsupport@example.com for assistance." as critical'
            exit 1
        fi
...
get_client_secret() {
    ...
    if [[ -z "$secret" ]]; then
        ...
        show_info_dialog \
            ...
            "This Mac is missing required IT credentials in its Keychain.\n\nPlease email **itsupport@example.com** to re-run the enrollment setup policy." \
...
request_bearer_token() {
    ...
    if [[ -z "$BEARER_TOKEN" || "$BEARER_TOKEN" == "None" ]]; then
        ...
        show_info_dialog \
            ...
            "Could not authenticate with Jamf Pro. Please email **itsupport@example.com** for assistance." \
...
show_selection_dialog() {
    ...
    if [[ ${#checkbox_args[@]} -eq 0 ]]; then
        ...
        show_info_dialog "No Logs Found" "No diagnostic logs were found on this Mac. Please email itsupport@example.com for assistance." "SF=exclamationmark.triangle.fill,color=orange"
        exit 1
    fi
...
get_jamf_id() {
    ...
    if [[ -z "$JSS_ID" ]]; then
        ...
        show_info_dialog \
            "Computer Not Found" \
            "This Mac could not be found in Jamf Pro. Please email **itsupport@example.com** for assistance." \
